...
void main() {
    ...
    // Calculate offset position for scrolling effect
    // Scale the y_offset to work in 3D space (convert from screen to world space)
    float worldOffset = -u_y_offset * 0.002;
    
    // Original displacement with y_offset applied
    ...
    // Apply y_offset to flowUv before scaling by color_pressure for smooth continuous scrolling
    vec2 adjustedUv = flowUv;
    adjustedUv.y -= u_y_offset * 0.0002;
...
void main() {
    ...
    if (u_enable_procedural_texture > 0.5) {
        ...
        // --- PARALLAX CONTROL ---
        // The vertex shader scrolls at approx 0.002.
        // To make the mask slower, we "undo" part of that scroll here.
        // Adding 0.0005 makes it 25% slower.
        // Adding 0.001 makes it 50% slower.
        texUv.y += u_y_offset * 0.0002; 
...
float snoise(vec3 v) {
  ...
  // Permutations
  // REMOVED: i = mod289(i); -> We don't wrap 'i' anymore.
...
float cnoise(vec3 P)
{
  ...
  // REMOVED: Pi0 = mod289(Pi0);
  // REMOVED: Pi1 = mod289(Pi1);
